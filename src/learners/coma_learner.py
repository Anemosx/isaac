...
class COMALearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, loss_ant=th.tensor(0.), negative_reward=False, custom_rewards=None):
        ...
        coma_loss = - ((advantages * log_pi_taken) * mask).sum() / mask.sum() + self.args.antagonist_loss * loss_ant
    ...
    def _train_critic(self, batch, rewards, terminated, actions, avail_actions, mask, bs, max_t, loss_ant):
        # Optimise critic
        ...
        for t in reversed(range(rewards.size(1))):
            ...
            loss = (masked_td_error ** 2).sum() / mask_t.sum() + self.args.antagonist_loss * loss_ant
