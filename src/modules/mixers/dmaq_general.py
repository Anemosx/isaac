...
from modules.mixers.qmix import UnconstrainedQMixer
...
class ConcatStateHistoryDMAQer(DMAQer):
    def __init__(self, args):
        self.state_dim = int(np.prod(args.state_shape))
        self.history_dim = int(np.prod(args.history_dim))
        self.state_dim += self.history_dim
        args.state_shape = self.state_dim
        super(ConcatStateHistoryDMAQer, self).__init__(args)

    def forward(self, agent_qs, states, histories, actions=None, max_q_i=None, is_v=False):
        bs = agent_qs.size(0)
        states = th.cat([states.view(bs, -1), histories.view(bs, -1)], dim=1)
        return super(ConcatStateHistoryDMAQer, self).forward(agent_qs, states, actions, max_q_i, is_v)

class DoubleDMAQer(nn.Module):
    def __init__(self, args):
        super(DoubleDMAQer, self).__init__()
        self.unconstrained_mixer = UnconstrainedQMixer(args)
        self.state_dim = np.prod(args.state_shape)
        self.n_agents = args.n_agents
        self.unconstrained_input_size = self.state_dim + self.n_agents
        args.state_shape = [args.rnn_hidden_dim, args.n_agents]
        self.constrained_mixer = DMAQer(args)

    def forward(self, agent_qs, states, hidden_states, actions=None, max_q_i=None, is_v=False):
        bs = agent_qs.size(0)
        states = states.reshape(bs, -1, self.state_dim)
        agent_qs = agent_qs.view(bs, -1, self.n_agents)
        return self.unconstrained_mixer(agent_qs.detach(), states), self.constrained_mixer(agent_qs, hidden_states, actions, max_q_i, is_v)